...
Strategy.prototype.authenticate = async function(req, options) {
 console.log('using custom passport-dapp-web3 for improve security');
 options = options || {};
 ...
 var nounce = '';
 await db.getObjectField('web3:address:uid', address, async (err, uid) => {
   if (err){
     nounce = null;
   }
   if(uid) {
     nounce = await user.getUserField(uid, 'nounce');
   }
   else {
     nounce = '';
   }
   var accounts = new ethAccounts();
   if (address !== accounts.recover(message+nounce, signed)) {
